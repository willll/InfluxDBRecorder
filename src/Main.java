...
public class Main implements ILogger {
	...
	private static class ticker_subscriber_task implements Runnable {
		...
		public ticker_subscriber_task(String ticker_pub) throws IOException {
			...
			InfluxDB influxDB = InfluxDBFactory.connect("http://" + influxdb_address + ":" + influxdb_port,
		...
		public void run() {
			...
			try {
				...
				logger.info("Listening : " + ticker_pub);
					...
					try {
						...
						for (String key : KEYS) {
							if (!obj.isNull(key) && !obj.isEmpty()) {
								influxDB.write(Point.measurement(currencies_pair).time(timestamp, TimeUnit.MILLISECONDS)
										.addField(key, obj.getDouble(key)).build());
							}
						}
						...
						for (String key : KEYS) {
							if (!obj.isNull(key) && !obj.isEmpty()) {
								influxDB.write(
										Point.measurement(currencies_pair).addField(key, obj.getDouble(key)).build());
							}
						}
					}
				}
